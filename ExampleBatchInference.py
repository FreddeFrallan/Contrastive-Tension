...
def torchExample():
    model = transformers.AutoModel.from_pretrained('Contrastive-Tension/RoBerta-Large-CT-STSb')
    tokenizer = transformers.AutoTokenizer.from_pretrained('Contrastive-Tension/RoBerta-Large-CT-STSb')

    embeddings = Inference.generateSentenceEmbeddings(model, tokenizer, random_texts, 'pt')
...
def tensorflowExample():
    ...
    embeddings = Inference.generateSentenceEmbeddings(model, tokenizer, random_texts, 'tf')
    print(embeddings.shape)
